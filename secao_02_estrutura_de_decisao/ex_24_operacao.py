"""
Exercício 24 da seção de estrutura de decisão da Python Brasil:
https://wiki.python.org.br/EstruturaDeDecisao

Faça um Programa que leia 2 números e em seguida pergunte ao usuário qual operação ele deseja realizar. O resultado da
operação deve ser acompanhado de uma frase que diga se o número é:
  par ou ímpar;
  positivo ou negativo;
  inteiro ou decimal.

Mostre o restultado com duas casas decimais

    >>> fazer_operacao_e_classificar(2, 3, '+')
    Resultado: 5.00
    Número é impar, positivo e inteiro.
    >>> fazer_operacao_e_classificar(-4, 2, '/')
    Resultado: -2.00
    Número é par, negativo e inteiro.
    >>> fazer_operacao_e_classificar(0, 2, '*')
    Resultado: 0.00
    Número é par, neutro e inteiro.
    >>> fazer_operacao_e_classificar(-3.14, 2, '*')
    Resultado: -6.28
    Número é negativo e decimal.
    >>> fazer_operacao_e_classificar(3.14, 2, '-')
    Resultado: 1.14
    Número é positivo e decimal.

"""


def fazer_operacao_e_classificar(n_1: float, n_2: float, operacao: str):
    """Escreva aqui em baixo a sua solução"""
    possibilidades = ['par', 'impar', 'positivo', 'negativo', 'neutro', 'inteiro', 'decimal']
    output = []

    if operacao == '+':
      resultado = n_1 + n_2
    if operacao == '/':
      resultado = n_1 / n_2
    if operacao == '*':
      resultado = n_1 * n_2
    if operacao == '-':
      resultado = n_1 - n_2

    if resultado % 1 == 0: #se for inteiro
      if resultado % 2 == 0:
        output.append(possibilidades[0])
      else:
          output.append(possibilidades[1])

      if resultado > 0:
        output.append(possibilidades[2])
        output.append(possibilidades[5])
      elif resultado < 0:
        output.append(possibilidades[3])
        output.append(possibilidades[5])
      else:
        output.append(possibilidades[4])
        output.append(possibilidades[5])
        
    else: #se for decimal
      if resultado > 0:
        output.append(possibilidades[2])
        output.append(possibilidades[6])
      elif resultado < 0:
        output.append(possibilidades[3])
        output.append(possibilidades[6])
      else:
        output.append(possibilidades[4])
        output.append(possibilidades[6])
        
    print(f'Resultado: {resultado:.2f}')
    if len(output) == 1:
      print(f'Número é {output[0]}.')
    elif len(output) == 2:
      print(f'Número é {output[0]} e {output[1]}.')
    elif len(output) == 3:
      print(f'Número é {output[0]}, {output[1]} e {output[2]}.')