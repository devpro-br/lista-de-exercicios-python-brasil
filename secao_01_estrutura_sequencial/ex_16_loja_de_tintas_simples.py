...
def calcular_latas_e_preco_de_tinta():
    ...
    metros = int(input("Quantos metros quadrados você vai pintar? "))
    litros_tinta = metros / 3
    latas = litros_tinta / 18
    custo = round(latas) * 80
    print(f"Você deve comprar {round(latas)} lata(s) tinta ao custo de R$ {custo:.2f}")
